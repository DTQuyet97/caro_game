...
void _Board::loadData(int i, int j, int k)
{
	...
	if (k == -1)
	{
		_Common::Textcolor(BC_Blue);	//X
		_Common::_Common::gotoXY(4 * j + _left + 2, 2 * i + _top + 1);
		cout << "X";
		CountX++;
	}
	if (k == 1)
	{
		_Common::Textcolor(BC_Red);	//O
		_Common::_Common::gotoXY(4 * j + _left + 2, 2 * i + _top + 1);
		cout << "O";
		CountY++;
	}
...
// Xet chien thang theo hang doc
int _Board::checkWinCol(int x, int y, int value) 
{
	int dong, cot;
	int test = -1, loop = 1;
	int check2dau = 0;
...
int _Board::testBoard(int x, int y)
{
	int SIZE = 16;
	// X chien thang
	if (checkWinRow(x, y, -1) == 1)		//check win hàng
		return -1;
	if (checkWinCol(x, y, -1) == 1)		//check win cot
		return -1;
